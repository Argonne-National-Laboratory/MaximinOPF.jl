...
# The master problem MP
 ...
 # Define the model here
  ...
  @constraint(mMP, [l in L], relFac*rho_p*(1 - x[l]) >= λF[l])
  @constraint(mMP, [l in L], α[toBus[l]] - relFac*rho_p*x[l] <= λT[l])
  @constraint(mMP, [l in L], α[toBus[l]] + relFac*rho_p*x[l] >= λT[l])
  @constraint(mMP, [l in L], -relFac*rho_p*(1 - x[l]) <= λT[l])
  @constraint(mMP, [l in L], relFac*rho_p*(1 - x[l]) >= λT[l])

  @constraint(mMP, [l in L], β[fromBus[l]] - relFac*rho_q*x[l] <= μF[l])
  @constraint(mMP, [l in L], β[fromBus[l]] + relFac*rho_q*x[l] >= μF[l])
  @constraint(mMP, [l in L], -relFac*rho_q*(1 - x[l]) <= μF[l])
  @constraint(mMP, [l in L], relFac*rho_q*(1 - x[l]) >= μF[l])
  @constraint(mMP, [l in L], β[toBus[l]] - relFac*rho_q*x[l] <= μT[l])
  @constraint(mMP, [l in L], β[toBus[l]] + relFac*rho_q*x[l] >= μT[l])
 ...
 # The dual SOCP constraints 
  @expression(mMP, wCoeff[i=N], 
    α[i]*Y["shR"][i] - β[i]*Y["shI"][i] + δ[i] - γ[i]
    	+ sum( λF[l]*Y["ffR"][l] - μF[l]*Y["ffI"][l] for l in fromLines[i])   
	+ sum( λT[l]*Y["ttR"][l] - μT[l]*Y["ttI"][l] for l in toLines[i])  
	)
  @expression(mMP, wRCoeff[l=L], λF[l]*Y["ftR"][l] - μF[l]*Y["ftI"][l] + λT[l]*Y["tfR"][l] - μT[l]*Y["tfI"][l])
  @expression(mMP, wICoeff[l=L], λF[l]*Y["ftI"][l] - λT[l]*Y["tfI"][l] + μF[l]*Y["ftR"][l] - μT[l]*Y["tfR"][l])

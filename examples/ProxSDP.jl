...
function solve_PSD_via_ADMM(model_info::Dict{String,Any}; max_n_iter=100, prox_t=1, io=Base.stdout)
    ...
    for ii=0:max_n_iter
        ...
        prim_res = trunc(sqrt(sum( sum( PSD[kk]["prim_res"][nn]^2 for nn in 1:PSD[kk]["vec_len"]) for kk in keys(PSD)));digits=4)
        dual_res = trunc(sqrt(sum( sum( PSD[kk]["dual_res"][nn]^2 for nn in 1:PSD[kk]["vec_len"]) for kk in keys(PSD)));digits=4)
        println("\tProx obj value: ",model_info["prox_val"]," in statu: ", model_info["solve_status"]," p_res=",prim_res," d_res=",dual_res )
        if sqrt(pd_res_val) < 1e-6
...
function solveProxPtSP(model_info)
    ...
    @objective( model_info["model"], model_info["obj_sense"], model_info["model"][:linobj_expr]  
        + 0.5* model_info["prox_sign"] * model_info["prox_t"] * sum( PSD[kk]["quad_terms"] for kk in keys(PSD) )
    )
...
function solve_PSD_via_ProxPt(model_info::Dict{String,Any}; max_n_iter=100, prox_t=1, io=Base.stdout)
    ...
    for ii=0:max_n_iter
        ...
        println("\t\tMin eig value is: ", minimum( PSD[kk]["eig_val"] for kk in keys(PSD)), 
            ",\tmin center eig value is: ", minimum( PSD[kk]["eig_val_ctr"] for kk in keys(PSD)))
...
function add_psd_initial_cuts(model_info; io=Base.stdout)
    psd_expr = model_info["model"][:psd_expr]
    PSD=model_info["psd_con"]
    #JuMP.@constraint( model_info["model"], psd_expr_lbs[kk in keys(PSD), mm in 1:PSD[kk]["vec_len"]], psd_expr[kk,mm] >= -PSD[kk]["is_off_diag"][mm]*1e3 )
    JuMP.@constraint( model_info["model"], psd_expr_lbs[kk in keys(PSD), mm in 1:PSD[kk]["vec_len"]], psd_expr[kk,mm] >= -1e3 )
#=
    for kk in model_info["psd_con_keys"]
    ...
    for kk in keys(PSD)
        ...
        if min_eigval < -1e-6
            ...
            #for mm in 1:1 # filter(mmm->(E[1][mmm] < 0), 1:n_eigs)
                ...
                for nn=1:n_el
	                if ii==jj
	                    PSD[kk]["sg"][nn] = eig_vecs[ii,mm]*eig_vecs[jj,mm]  
                        jj += 1
                        ii = 1
	                else
	                    PSD[kk]["sg"][nn] = (eig_vecs[ii,mm]*eig_vecs[jj,mm] + eig_vecs[jj,mm]*eig_vecs[ii,mm]) 
                        ii += 1
	                end
                end
                ncuts=length(PSD[kk]["new_cuts"])
                PSD[kk]["new_cuts"][ncuts] = Dict{String,Any}()
                PSD[kk]["new_cuts"][ncuts]["ref"] = @constraint(model_info["model"], sum( PSD[kk]["sg"][nn]*psd_expr[kk,nn] for nn=1:n_el ) >= 0) 
            #end
        end
    end
    return new_cut
...
function PSDProjection(model_info; use_most_neg_ev=true, io=Base.stdout)
    ...
    for kk in keys(model_info["psd_con"])
        ...
        PSD["neg_eigs_sum"] = 0.0
        for mm in neg_eigs
            PSD["neg_eigs_sum"] += eig_vals[mm]
            ii,jj=1,1
            for nn=1:n_el
        ...
        if PSD["neg_eigs_sum"] < -1e-8
            is_nontrivial_projection = true
            if use_most_neg_ev
                ii,jj=1,1
                for nn=1:n_el
                    PSD["sg"][nn] = eig_vecs[ii,min_eigval_idx]*eig_vecs[jj,min_eigval_idx]
	                if ii==jj
                        jj += 1
                        ii = 1
	                else
                ...
                for nn=1:n_el
                    PSD["sg"][nn] = (1.0/PSD["neg_eigs_sum"])*(PSD["expr_val"][nn]-PSD["proj_expr_val"][nn])
                end
            end
        end
    end
...
function ADMMProjections(model_info;io=Base.stdout)
    ...
    for kk in keys(model_info["psd_con"])
        ...
        for nn=1:n_el
            ...
            PSD["dual_res"][nn] = PSD["proj_expr_val"][nn] - PSD["old_proj_expr_val"][nn]
        end
    end
    return is_nontrivial_projection
end

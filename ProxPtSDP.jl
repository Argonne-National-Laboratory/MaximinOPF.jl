...
=#
  ...
  for i in N
   for g in BusGeners[i]
	...
	+ sum( γm[i]*Wmin[i]-γp[i]*Wmax[i] + α[i]*PD[i] + β[i]*QD[i] for i in N) - tVal*tau
    ...
    @constraint(mMP, alphaProxUB[i=N], α_ctr[i] - α[i] <= tau)
    @constraint(mMP, alphaProxLB[i=N], α_ctr[i] - α[i] >= -tau)
    @constraint(mMP, betaProxUB[i=N], β_ctr[i] - β[i] <= tau)
    @constraint(mMP, betaProxLB[i=N], β_ctr[i] - β[i] >= -tau)
    @constraint(mMP, gammaProxUB[i=N], γ_ctr[i] - γm[i] <= tau)
    @constraint(mMP, gammaProxLB[i=N], γ_ctr[i] - γm[i] >= -tau)
    @constraint(mMP, deltaProxUB[i=N], δ_ctr[i] - γp[i] <= tau)
    @constraint(mMP, deltaProxLB[i=N], δ_ctr[i] - γp[i] >= -tau)
...
function testECP()
   ...
   for kk=1:MAX_N_ITERS
	...
	dNorm = sqrt(norm(α_val-α_ctr)^2 + norm(β_val-β_ctr)^2 + norm(γ_val-γ_ctr)^2 + norm(δ_val-δ_ctr)^2 
        ...
        if ( (1.0*tVal)*dNorm <= TOL && -etaCtr <= TOL ) 
	   break
        end
...
function solveEta0Eigs(firstIt=false)
	...
	global nSG, iter
	global η0Val, etaCtr, tVal, tCntr, epsMin, tMAX, tMIN, objval
	global α_val, β_val, γ_val, δ_val, linobjval
	global ζpUB_val, ζpLB_val, ζqUB_val, ζqLB_val
	global α_ctr, β_ctr, γ_ctr, δ_ctr, linobj_ctr
	global α_trl, β_trl, γ_trl, δ_trl, linobj_trl, etaTrl
	global ζpUB_ctr, ζpLB_ctr, ζqUB_ctr, ζqLB_ctr
	  ...
	  if nCuts == MAX_N_CUTS
	    ...
	    if dualSum > 1e-6
	      for i in N
	       ...
	       δ_trl[i,1] /= dualSum
	       γ_trl[i,1] /= dualSum
	       linobj_trl[1] = sum( cutDuals[s]*linobj_trl[s] for s in 1:nCuts)
	       linobj_trl /= dualSum
	      end
	      nCuts = 1
	    else
...
########  Adjust penalty under serious step ###############
	        hk = 2*tVal0*(1.0 - SSCVal)
		tProv = tVal0
		if SSCVal >= min(2*SSC,1) && tCntr > 0
		  tProv = hk
		elseif tCntr > 3
		  tProv = 0.5*tVal0
		end
...
@show objval,linobjval,etaCtr,iter,tVal,tCntr,SSCVal
	  else
...
########  Adjust penalty under null step ###############
	    ...
	    tProv = tVal0
	    epsMin = min(epsMin, vVal + tVal0*proxTermNorm^2 + proxTermNorm)
	    #if -linErrorVal > max( -etaCtr + tVal0*proxTermNorm^2 + proxTermNorm, -10*vVal) && tCntr < -3
	    if -linErrorVal > max( epsMin, -10*etaCtr) && tCntr < -3
		tProv = hk
	    end
